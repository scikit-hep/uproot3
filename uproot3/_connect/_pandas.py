...
import awkward0 as awkwardbase
...
def futures2df(futures, outputtype, entrystart, entrystop, flatten, flatname, awkward0):
    import pandas
    ...
    if not flatten or all(interpretation.__class__ is not asjagged for name, interpretation, future in futures):
        ...
        for name, interpretation, future in futures:
            ...
            if isinstance(interpretation, asjagged):
                ...
                if starts is None:
                    ...
                    if starts is not array.starts and not awkward0.numpy.array_equal(starts, array.starts):
        ...
        for name, interpretation, array, needbroadcast in zip(names, interpretations, arrays, needbroadcasts):
            if isinstance(interpretation, uproot3.interp.numerical._asnumeric):
                ...
                if needbroadcast:
                    ...
                    if isinstance(array, awkwardbase.Table):
                        for nn in array.columns:
                            array[nn] = awkward0.JaggedArray(starts, stops, awkward0.numpy.empty(stops[-1], dtype=array[nn].dtype)).tojagged(array[nn]).content

                    else:
                        if len(originaldims) != 0:
                            array = array.view(awkward0.numpy.dtype([(str(i), array.dtype) for i in range(functools.reduce(operator.mul, array.shape[1:]))])).reshape(array.shape[0])

                        array = awkward0.JaggedArray(starts, stops, awkward0.numpy.empty(stops[-1], dtype=array.dtype)).tojagged(array).content
                        if len(originaldims) != 0:
                ...
                array = awkward0.numpy.array(array, dtype=object)
                indexes = numpy.arange(len(array))

                indexes = awkward0.JaggedArray(starts, stops, awkward0.numpy.empty(stops[-1], dtype=object)).tojagged(indexes).content

                array = array[indexes]

                if len(array) != 0 and isinstance(array[0], awkward0.numpy.ndarray):
                    df[fn] = list(array)
                else:
                    df[fn] = array
