        ...
        if len(args) == 0:
            try:
                ...
                if read_streamers and fSeekInfo != 0:
                    streamercontext = ROOTDirectory._FileContext(source.path, None, None, streamerclasses, uproot3.source.compressed.Compression(fCompress), tfile)
                ...
                context = ROOTDirectory._FileContext(source.path, streamerinfos, streamerinfosmap, classes, uproot3.source.compressed.Compression(fCompress), tfile)
...
def _startcheck(source, cursor):
    ...
    if numpy.int64(cnt) & uproot3.const.kByteCountMask:
        cnt = int(numpy.int64(cnt) & ~uproot3.const.kByteCountMask)
...
def _skiptobj(source, cursor):
    ...
    if numpy.int64(version) & uproot3.const.kByteCountVMask:
        cursor.skip(4)
    fUniqueID, fBits = cursor.fields(source, _skiptobj._format2)
    fBits = numpy.uint32(fBits) | uproot3.const.kIsOnHeap
    if fBits & uproot3.const.kIsReferenced:
        cursor.skip(2)
...
def _readobjany(source, cursor, context, parent, asclass=None):
    ...
    if numpy.int64(bcnt) & uproot3.const.kByteCountMask == 0 or numpy.int64(bcnt) == uproot3.const.kNewClassTag:
        vers = 0
        start = 0
        tag = bcnt
        bcnt = 0
    ...
    if numpy.int64(tag) & uproot3.const.kClassMask == 0:
        # reference object
        if tag == 0:
    ...
    elif tag == uproot3.const.kNewClassTag:
        ...
        if vers > 0:
            cursor.refs[start + uproot3.const.kMapOffset] = fct
        else:
        ...
        if vers > 0:
            cursor.refs[beg + uproot3.const.kMapOffset] = obj
        else:
        ...
        ref = int(numpy.int64(tag) & ~uproot3.const.kClassMask)
    ...
    for obj in tlist:
        if isinstance(obj, TStreamerInfo):
            ...
            for element in obj._fElements:
                ...
                # if isinstance(element, (TStreamerObject, TStreamerObjectAny, TStreamerString)) or (isinstance(element, TStreamerObjectPointer) and element._fType == uproot3.const.kObjectp):
...
def _ftype2dtype(fType):
    ...
    elif fType in (uproot3.const.kUChar, uproot3.const.kCharStar):
    ...
    elif fType in (uproot3.const.kFloat, uproot3.const.kFloat16):
        return "numpy.dtype('>f4')"
    elif fType in (uproot3.const.kDouble, uproot3.const.kDouble32):
        ...
        if isinstance(streamerinfo, TStreamerInfo) and pyclassname not in builtin_classes and (pyclassname not in classes or hasattr(classes[pyclassname], "_versions")):
            ...
            for elementi, element in enumerate(streamerinfo._fElements):
                if isinstance(element, TStreamerArtificial):
                    ...
                    assert uproot3.const.kOffsetP < element._fType < uproot3.const.kOffsetP + 20
                    fType = element._fType - uproot3.const.kOffsetP
                    ...
                    if element._fType == uproot3.const.kObjectp or element._fType == uproot3.const.kAnyp:
                    ...
                    elif element._fType == uproot3.const.kObjectP or element._fType == uproot3.const.kAnyP:
                        if pyclassname in skip and _safename(element._fName) in skip[pyclassname]:
                    ...
                    if element._fSTLtype == uproot3.const.kSTLstring or element._fTypeName == b"string":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.string(source)".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kBool) or element._fTypeName == b"vector<bool>" or element._fTypeName == b"vector<Bool_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '?')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kChar) or element._fTypeName == b"vector<char>" or element._fTypeName == b"vector<Char_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), 'i1')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kUChar) or element._fTypeName == b"vector<unsigned char>" or element._fTypeName == b"vector<UChar_t>" or element._fTypeName == b"vector<Byte_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), 'u1')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kShort) or element._fTypeName == b"vector<short>" or element._fTypeName == b"vector<Short_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>i2')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kUShort) or element._fTypeName == b"vector<unsigned short>" or element._fTypeName == b"vector<UShort_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>u2')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kInt) or element._fTypeName == b"vector<int>" or element._fTypeName == b"vector<Int_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>i4')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kUInt) or element._fTypeName == b"vector<unsigned int>" or element._fTypeName == b"vector<UInt_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>u4')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kLong) or element._fTypeName == b"vector<long>" or element._fTypeName == b"vector<Long_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>i8')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kULong) or element._fTypeName == b"vector<unsigned long>" or element._fTypeName == b"vector<ULong64_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>u8')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kFloat) or element._fTypeName == b"vector<float>" or element._fTypeName == b"vector<Float_t>":
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = cursor.array(source, cursor.field(source, self._int32), '>f4')".format(_safename(element._fName)))
                        fields.append(_safename(element._fName))
                    elif (element._fSTLtype == uproot3.const.kSTLvector and element._fCtype == uproot3.const.kDouble) or element._fTypeName == b"vector<double>" or element._fTypeName == b"vector<Double_t>":
                        code.append("        cursor.skip(6)")
                        ...
                        code.append("        cursor.skip(6)")
                        code.append("        self._{0} = uproot3.interp.objects.STLVector(uproot3.interp.objects.STLString()).read(source, cursor, context, self)".format(_safename(element._fName)))
...
class TKey(ROOTObject):
    ...
    def _readinto(cls, self, source, cursor, context, parent):
        ...
        if self._fObjlen != self._fNbytes - self._fKeylen:
            self._source = uproot3.source.compressed.CompressedSource(context.compression, source, Cursor(self._fSeekKey + self._fKeylen), self._fNbytes - self._fKeylen, self._fObjlen)
...
class TStreamerBasicType(TStreamerElement):
    ...
    def _readinto(cls, self, source, cursor, context, parent):
        ...
        if uproot3.const.kOffsetL < self._fType < uproot3.const.kOffsetP:
            self._fType -= uproot3.const.kOffsetL

        basic = True
        if self._fType in (uproot3.const.kBool, uproot3.const.kUChar, uproot3.const.kChar):
            self._fSize = 1
        elif self._fType in (uproot3.const.kUShort, uproot3.const.kShort):
            self._fSize = 2
        elif self._fType in (uproot3.const.kBits, uproot3.const.kUInt, uproot3.const.kInt, uproot3.const.kCounter):
            self._fSize = 4
        elif self._fType in (uproot3.const.kULong, uproot3.const.kULong64, uproot3.const.kLong, uproot3.const.kLong64):
            self._fSize = 8
        elif self._fType in (uproot3.const.kFloat, uproot3.const.kFloat16):
            self._fSize = 4
        elif self._fType in (uproot3.const.kDouble, uproot3.const.kDouble32):
            self._fSize = 8
        elif self._fType == uproot3.const.kCharStar:
...
class TStreamerSTL(TStreamerElement):
    ...
    def vector(cls, fType, fTypeName):
        ...
        self._fSTLtype = uproot3.const.kSTLvector
        self._fCtype = fType
