...
from uproot3.write.TKey import BasketKey
...
class TTree(object):
    ...
    def extend(self, branchdict):
        ...
        for key, value in branchdict.items():
            if not isinstance(value, awkward0.array.jagged.JaggedArray):
                branchdict[key] = numpy.array(value, dtype=self._branches[key]._branch.type, copy=False)

        for key, value in branchdict.items():
            if isinstance(value, awkward0.array.jagged.JaggedArray):
    ...
    @property
    def numentries(self):
        t = uproot3.open(self._file._path)[self.name]
        return t.numentries
...
class TBranch(object):
    ...
    def newbasket(self, items, multidim=None):
        ...
        if self._branch.fields["_fWriteBasket"] >= self._branch.fields["_fMaxBaskets"]:
            ...
            cursor = uproot3.write.sink.cursor.Cursor(self._branch.file._fSeekFree)
            tree.write_key = uproot3.write.TKey.TKey(fClassName=self._treelvl1._tree.write_key.fClassName,
            ...
            tree.keycursor = uproot3.write.sink.cursor.Cursor(tree.write_key.fSeekKey)
        ...
        if isinstance(items, awkward0.array.jagged.JaggedArray):
            ...
            key = BasketKey(fName=self._branch.name,
                            ...
                            fSeekPdir=copy(self._branch.file._fBEGIN),
                            fBufferSize=32000)
        keycursor = uproot3.write.sink.cursor.Cursor(key.fSeekKey)
        key.write(cursor, self._branch.file._sink)
        uproot3.write.compress.write(self._branch.file, cursor, givenbytes, self._branch.compression, key, copy(keycursor))
        if isinstance(items, awkward0.array.jagged.JaggedArray):
            ...
            uproot3.write.compress.write(self._branch.file, cursor, offsetbytes, self._branch.compression, key,
    ...
    def interpretation(self):
        b = uproot3.open(self._treelvl1._file._path)[self._treelvl1.name][self.name]
...
class TTreeImpl(object):
    ...
    def write(self, context, cursor, name, key, keycursor, util):
        ...
        uproot3.write.compress.write(context, copy(self.tree_write_cursor), givenbytes, None, key, copy(self.write_keycursor))
