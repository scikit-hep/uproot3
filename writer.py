...
class Writer(object):
    ...
    def __setitem__(self, keyname, item):
        ...
        if type(item) is TH1F:
            junkkey = TH1FJunkKey(keyname.encode("utf-8"), item.fNbins, item.fName, item.fTitle)
            key = TH1FKey(keyname.encode("utf-8"), pointcheck, item.fNbins, item.fName, item.fTitle)
