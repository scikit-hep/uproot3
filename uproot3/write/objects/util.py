...
class Util(object):
    ...
    def _putclass(self, cursor, obj, keycursor, beg):
        ...
        if clsname in self._written:
            buf += cursor.put_fields(self._format_putobjany1, self._written[clsname] | uproot3.const.kClassMask)
            if clsname != "TBranch":
                self._written[id(objct)] = beg + uproot3.const.kMapOffset
        else:
            buf += cursor.put_fields(self._format_putobjany1, uproot3.const.kNewClassTag)
            buf += cursor.put_cstring(clsname)
            self._written[clsname] = numpy.uint32(start + uproot3.const.kMapOffset) | uproot3.const.kClassMask
            self._written[id(objct)] = beg + uproot3.const.kMapOffset
    ...
    def put_objany(self, cursor, obj, keycursor):
        ...
        if id(objct) in self._written and clsname in self._written:
            ...
            buff = copy_cursor.put_fields(self._format_putobjany1, numpy.uint32(len(class_buf)) | uproot3.const.kByteCountMask)
        else:
            copy_cursor = copy(cursor)
            cursor.skip(self._format_putobjany1.size)
