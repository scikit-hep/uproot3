...
def _flatlen(obj, awkward0):
    if isinstance(obj, awkward0.numpy.dtype):
        dtype, shape = _dtypeshape(obj)
        return int(awkward0.numpy.prod(shape))
    else:
        return int(awkward0.numpy.prod(obj.shape))

class _asnumeric(uproot3.interp.interp.Interpretation):
    @property
    ...
    def type(self):
        ...
        if shape == ():
            ...
            return self.awkward0.type.ArrayType(*(shape + (dtype,)))

    def empty(self):
        return self.awkward0.numpy.empty(0, self.todtype)

    def source_numitems(self, source):
        return _flatlen(source, self.awkward0)

    def destination(self, numitems, numentries):
        quotient, remainder = divmod(numitems, _flatlen(self.todtype, self.awkward0))
        if remainder != 0:
            raise ValueError("cannot reshape {0} items as {1} (i.e. groups of {2})".format(numitems, self.todtype.shape, _flatlen(self.todtype, self.awkward0)))
        return self.awkward0.numpy.empty(quotient, dtype=self.todtype)
    ...
    def clip(self, destination, itemstart, itemstop, entrystart, entrystop):
        length = _flatlen(self.todtype, self.awkward0)
...
class asdtype(_asnumeric):
    ...
    def __init__(self, fromdtype, todtype=None):
        if isinstance(fromdtype, self.awkward0.numpy.dtype):
            ...
            self.fromdtype = self.awkward0.numpy.dtype(fromdtype)
        elif isinstance(fromdtype, list) and any(e[1][0] in BYTEORDER_INDICATORS for e in fromdtype):
            self.fromdtype = self.awkward0.numpy.dtype(fromdtype)
        else:
            self.fromdtype = self.awkward0.numpy.dtype(fromdtype).newbyteorder(">")
        ...
        if todtype is None:
            ...
            self.todtype = self.awkward0.numpy.dtype(todtype)
        elif isinstance(todtype, list) and any(e[1][0] in BYTEORDER_INDICATORS for e in todtype):
            self.todtype = self.awkward0.numpy.dtype(todtype)
        else:
            self.todtype = self.awkward0.numpy.dtype(todtype).newbyteorder("=")
    ...
    def to(self, todtype=None, todims=None):
        ...
        return asdtype(self.fromdtype, self.awkward0.numpy.dtype((dtype, shape)))
    ...
    @property
    def identifier(self):
        _byteorder = {"!": "B", ">": "B", "<": "L", "|": "L", "=": "B" if self.awkward0.numpy.dtype(">f8").isnative else "L"}
        def form(dt, n):
...
class asarray(asdtype):
    ...
    @property
    def todtype(self):
        return self.awkward0.numpy.dtype((self.toarray.dtype, self.toarray.shape[1:]))
    ...
    def destination(self, numitems, numentries):
        ...
        if remainder != 0:
            raise ValueError("cannot reshape {0} items as {1} (i.e. groups of {2})".format(numitems, self.todtype.shape, _flatlen(self.todtype, self.awkward0)))
        if _flatlen(self.toarray, self.awkward0) < numitems:
            raise ValueError("cannot put {0} items into an array of {1} items".format(numitems, _flatlen(self.toarray, self.awkward0)))
...
class asdouble32(_asnumeric):
    ...
    @property
    def todtype(self):
        return self.awkward0.numpy.dtype((self.awkward0.numpy.float64, self.todims))

    @property
    def fromdtype(self):
        if self.truncated:
            return self.awkward0.numpy.dtype(({'exponent': ('>u1', 0), 'mantissa': ('>u2', 1)}, self.fromdims))
        else:
            return self.awkward0.numpy.dtype(('>u4', self.fromdims))
    ...
    def fromroot(self, data, byteoffsets, local_entrystart, local_entrystop, keylen):
        ...
        if self.fromdims != ():
            product = int(self.awkward0.numpy.prod(self.fromdims))
        ...
        if self.truncated:
            ...
            unpacked = array['exponent'].astype(self.awkward0.numpy.int32)
            mantissa = array['mantissa'].astype(self.awkward0.numpy.int32)

            unpacked <<= 23
            unpacked |= (mantissa & ((1 << (self.numbits + 1)) - 1)) << (23 - self.numbits)
            sign = ((1 << (self.numbits + 1)) & mantissa != 0) * -2 + 1

            array = unpacked.view(dtype=self.awkward0.numpy.float32) * sign
            array = array.astype(self.todtypeflat)
        else:
            array = array[local_entrystart:local_entrystop].astype(self.todtypeflat)
            self.awkward0.numpy.multiply(array, float(self.high - self.low) / (1 << self.numbits), out=array)
            self.awkward0.numpy.add(array, self.low, out=array)
...
class asfloat16(asdouble32):
    ...
    @property
    def todtype(self):
        return self.awkward0.numpy.dtype((self.awkward0.numpy.float32, self.todims))
...
class asstlbitset(uproot3.interp.interp.Interpretation):
    # makes __doc__ attribute mutable before Python 3.3
    __metaclass__ = type.__new__(type, "type", (uproot3.interp.interp.Interpretation.__metaclass__,), {})

    @property
    def todtype(self):
        return self.awkward0.numpy.dtype(self.awkward0.numpy.bool_)
    ...
    @property
    def type(self):
        return self.awkward0.type.ArrayType(self.numbytes, self.todtype)

    def empty(self):
        return self.awkward0.numpy.empty((0, self.numbytes), dtype=self.todtype)
    ...
    def source_numitems(self, source):
        return int(self.awkward0.numpy.prod(source.shape))

    def fromroot(self, data, byteoffsets, local_entrystart, local_entrystop, keylen):
        return data.view(self.todtype).reshape((-1, self.numbytes + 4))[:, 4:]

    def destination(self, numitems, numentries):
        return self.awkward0.numpy.empty((numitems, self.numbytes), dtype=self.todtype)
